...
export function useKycForm() {
  ...
  const [submittedData, setSubmittedData] = useState<KycFormValues | null>(null);
  ...
  function onValid(values: KycFormValues) {
    ...
    values.documents.forEach((doc, i) => {
      ...
      if (doc.idNumber) formData.append(`documents[${i}].idNumber`, doc.idNumber);
      if (doc.issueDate) formData.append(`documents[${i}].issueDate`, doc.issueDate);
      if (doc.expiryDate) formData.append(`documents[${i}].expiryDate`, doc.expiryDate);
      if (doc.informantFirstName) formData.append(`documents[${i}].informantFirstName`, doc.informantFirstName);
      if (doc.informantMiddleName) formData.append(`documents[${i}].informantMiddleName`, doc.informantMiddleName);
      if (doc.informantLastName) formData.append(`documents[${i}].informantLastName`, doc.informantLastName);
      if (doc.informantRelationship) formData.append(`documents[${i}].informantRelationship`, doc.informantRelationship);
    });
  ...
  function onInvalid(errors: FieldErrors<KycFormValues>) {
    if (errors.documents) {
      toast.error("One of your documents is missing required info — check the front/back image and ID number.");
    } else {
